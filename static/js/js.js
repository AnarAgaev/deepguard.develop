...
var wndw = {
	...
	next: function(){
				// setTimeout(function() {
					...
					if(have_next){
						...
						// gtag('event', step_next+'_question', {
						...
						console.log('Захват номера телефона');

						setTimeout(function(){
							$(".chekboxs label").last().click()
			...
			go_first: function(){
				...
				console.log('Второе место - удаление класса')
	...
	ready(function(){
		...
		$(document).on("click", ".btn-send-test", function(){
			...
			console.log('Выводим результаты');

			// Отправляем форму на обработчик
			$("#test form.ajax").submit();
